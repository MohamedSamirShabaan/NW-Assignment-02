...
void recv_file(int sock)
{
    ...
    while(true)
    {
        ...
        if(pck_file.len==0)break;

        if(!validateCheckSum(pck_file))continue;
           ...
           cout << ack.ackno << "\n";
            if(pck_file.seqno>=rcv_base)
            {
                if(pck_file.seqno>=received.size())
                {
                    ...
                    file.resize(pck_file.seqno+1,pck_file);
                }
                received[pck_file.seqno]=true;
                file[pck_file.seqno]=pck_file;
            }
        }
    ...
    clock_t after = clock();
    time_taken = (after- before) / (double) CLOCKS_PER_SEC;
...
double open_connection(int port,string server_ip, string file_name,int window)
{
